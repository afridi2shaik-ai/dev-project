...
async def run_plivo_bot(websocket: WebSocket, stream_id: str, call_id: str | None, session_id: str, tenant_id: str, agent: BaseAgent, start_message: dict):
    logger.info(f"Starting bot for stream: {stream_id}")
    provider_session_id = call_id or stream_id
    ...
    if krisp_filter:
        ...
        logger.info(f"🎤 Using audio filter: {filter_type} for Plivo transport (8kHz)")
    else:
        logger.info("🎤 No audio filter configured for Plivo transport")
    ...
    await run_pipeline(transport=transport, agent=agent, session_id=session_id, tenant_id=tenant_id, provider_session_id=provider_session_id, transport_name="plivo", pipeline_params=pipeline_params, metadata=start_message)
